...
# 针对第二点，以买家库为主库，通过binlog+canal+mycat的方式，将买家库的订单数据异步同步至卖家库中，
# 再通过卖家id进行hash实现的分库分表。保证同一个卖家订单落到同一个卖家库的同一张表。卖家查询自己的订单，功能上也可以增加时间范围，条数的限制。
...
# 这里可以引入elasticSearch搜索引擎，将买家库的订单数据通过canal同步至es中，
# 再针对订单类型，时间, 订单尾号，买家姓名等字段创建合适的索引后，进行搜索也能最大化利用elasticSearch查询。
